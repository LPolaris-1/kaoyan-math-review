...
export function buildDailyQueue(items, progressById, today = shanghaiToday(), reviewedTodayIds = []) {
  const reviewedToday = reviewedTodayIds instanceof Set
    ? reviewedTodayIds
  ...
  const active = entries.filter(({ item, progress }) =>
    !progress.mastered && !reviewedToday.has(item.id)
  );
  ...
  // Keep the queue order explicit: yesterday's intake first, then due today,
  // overdue, older intake items, and finally future scheduled dates.
  return queue.sort((a, b) => {
    const rank = (entry) => {
      if (entry.source === "intake" && daysBetween(entry.item.date, today) === 1) return 0;
      if (entry.progress.cycleStartedAt && entry.progress.nextReviewDate === today) return 1;
      if (entry.progress.cycleStartedAt && entry.progress.nextReviewDate < today) return 2;
      if (entry.source === "intake") return 3;
      return 4;
    };
    const aRank = rank(a);
    const bRank = rank(b);
    ...
    // Intake items are first-review work, with the newest import first.
    // Returning zero for same-day imports preserves history.json order for that tie.
...
export function buildTodayProgress(queueEntries, events, today = shanghaiToday(), initialQueueIds) {
  ...
  // A user can continue reviewing after the queue has changed, so include
  // reviewed items outside the current or initial queue in today's total.
  const total = new Set([...ids, ...completed]).size;
  return {
